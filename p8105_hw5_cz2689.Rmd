...
```
Note: I noticed that the state variable for one Tulsa city was coded as AL, so I changed it here.
...
```
The effect size is the difference between the true mean and mean from the null (0), as our true mean increase in this example from 1 to 6, the effective size also increases. Thus, we can conclude that as effective size increases power (proportion of times the null was rejected) increases. 
...
```
As we can see from the plot, only when the then true mean is larger than 4, the sample average of rejected mean is close to the true mean. This is due to the fact that at larger true mean we have larger power.
